...
interface CreateChallengeDialogProps {
  onChallengeCreate: (challenge: { name: string; startDate: string; endDate: string; targetBooks: number }) => void;
}

export function CreateChallengeDialog({ onChallengeCreate }: CreateChallengeDialogProps) {
  ...
  const handleCreateChallenge = () => {
    if (!newChallenge.name.trim() || !newChallenge.startDate || !newChallenge.endDate || newChallenge.targetBooks <= 0) {
      toast({
        title: "Error",
        ...
        <div className="space-y-4 mt-4">
          ...
          <div>
            ...
            <Input
              ...
              onChange={(e) => setNewChallenge(prev => ({ ...prev, targetBooks: parseInt(e.target.value) || 0 }))}
            />
          </div>
